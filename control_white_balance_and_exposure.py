...
sh("v4l2-ctl -c white_balance_automatic=0")
sh("v4l2-ctl -c white_balance_temperature={}".format(white_balance_temperature_value))
...
while(True):
    ...
    if key == ord('q'):
        ...
        sh("v4l2-ctl -c white_balance_temperature={}".format(white_balance_temperature_value))
        sh("v4l2-ctl -l | grep white_balance_temperature")
    elif key == ord('d'):
        ...
        sh("v4l2-ctl -c exposure_time_absolute={}".format(exposure_value))
        sh("v4l2-ctl -l | grep exposure_time_absolute")
    elif key == ord('c'):
        if exposure_value > 0:
            exposure_value -= exposure_step
        print(exposure_value)
